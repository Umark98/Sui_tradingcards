module tradingcard::gadget_gameplay_items {
    ...
    public fun edit_metadata<T>(
        ...
        assert!(version > metadata.version, E_INVALID_VERSION);
    ...
    public fun get_media_urls<T>(
        asset: &GadgetGameplayItem<T>,
        metadata: &GadgetGameplayItemMetadata<T>
    ): (String, String) {
        let level = asset.level;
        let media_url_primary = *metadata.media_urls_primary.get(&level);
        let media_url_display = *metadata.media_urls_display.get(&level);
        (media_url_primary, media_url_display)
    }

    public fun update_item_images<T>(
        _: &AdminCap,
    ...
    fun create_vecmap<V: copy + drop>(keys: vector<u16>, values: vector<V>): VecMap<u16, V> {
        ...
        let mut i = 0;
        while (i < len) {
            vec_map::insert(&mut data, *vector::borrow(&keys, i), *vector::borrow(&values, i));
            i = i + 1;
        };
        data
    }
