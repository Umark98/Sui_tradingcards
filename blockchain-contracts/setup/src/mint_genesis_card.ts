...
async function mintGenesisCard({ cardType, mintNumber, recipient }: MintGenesisCardParams) {
  ...
  tx.moveCall({
    target: `${PACKAGE_ID}::trading_card_genesis::mint_and_transfer`,
    typeArguments: [`${PACKAGE_ID}::trading_card_genesis::${cardType}`],
    arguments: [
...
async function main() {
  try {
    ...
    // Example: Mint CommemorativeCard1 with mint number 1
    const result = await mintGenesisCard({
      cardType: 'CommemorativeCard1',
      mintNumber: 1,
