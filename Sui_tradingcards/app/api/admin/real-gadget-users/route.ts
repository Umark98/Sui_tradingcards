...
// All 165 gadget types from your Move contract
const GADGET_TYPES = [
...
export async function GET(request: Request) {
    try {
        ...
        try {
            ...
            let whereClause = "WHERE n.nft_title IN (";
            ...
            if (gadgetType) {
                whereClause += ` AND n.nft_title = $${GADGET_TYPES.length + 1}`;
            }
        ...
        console.error('Error fetching real gadget users:', error);
        return NextResponse.json(
            { 
                success: false,
                error: 'Failed to fetch real gadget users', 
