...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error reading genesis displays:', error);
    return NextResponse.json(
      { error: 'Failed to read genesis displays' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Genesis display saved for ${cardType}:`, displayId);

    return NextResponse.json({
      success: true,
      displayId,
      cardType,
      message: `Genesis display saved for ${cardType}`
    });

  } catch (error) {
    console.error('Error saving genesis display:', error);
    return NextResponse.json(
      { error: 'Failed to save genesis display', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
